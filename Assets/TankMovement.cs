...
public class TankMovement : MonoBehaviour, ISelectableUnit
{
    ...
    [Header("Detección de Terreno (Capas)")]
    ...
    void Start()
    {
        // Configuración física para movimiento Top-Down
        rb = GetComponent<Rigidbody2D>();
        rb.gravityScale = 0;
        rb.drag = 0;
        rb.freezeRotation = true;

        // Obtener componentes automáticamente si no se asignaron
    ...
    public void MoverADestino(Vector3 destino)
    {
        ...
        if (estaSeleccionado)
        {
            ...
            {
                Debug.Log("Orden inválida: Destino es agua o vacío.");
            }
        }
    }
    ...
    void CalcularRutaInteligente(Vector3 inicio, Vector3 destino)
    {
        ...
        // PASO 1: Comprobar si hay agua en línea recta
    ...
    void Mover()
    {
        ...
        // Actualizar visuales (rotación de orugas/torreta)
        if (tankVisuals != null)
        {
    ...
    // =========================================================
    //              UTILIDADES DE DETECCIÓN (EL FIX)
    // =========================================================

    // Comprueba paso a paso si una línea cruza la capa de agua
    bool CaminoTieneAgua(Vector3 inicio, Vector3 fin)
    {
        float distancia = Vector3.Distance(inicio, fin);
        if (distancia < 0.1f) return false;

        // Cuantas más muestras, más precisa es la detección (cada 0.5 metros)
    ...
    WaypointPuente EncontrarPuenteSimple(Vector3 inicio, Vector3 destino)
    {
        ...
        foreach (Collider2D collider in todosWaypoints)
        {
            ...
            // Verificamos que sea una entrada válida y tenga salida conectada
            if (waypoint != null && waypoint.waypointConectado != null)
            {
                ...
                {
                    // Última comprobación: ¿Podemos llegar a la entrada del puente sin agua?
    ...
    {
        // Un click es válido si es Suelo O Puente

...
public class EnemyTankController : MonoBehaviour
{
    ...
    public float velocidad = 2.5f; // Los tanques suelen ser un poco más lentos
    ...
    public void SetTarget(Vector3 posicionDestino)
    {
            ...
            Time.time - ultimoRecalculoTime > RECALCULO_INTERVALO)
        {
            ...
            // VERIFICACIÓN 1: ¿Es suelo válido?
            if (EsSueloValido(posicionDestino))
            {
                // VERIFICACIÓN 2: ¿Hay agua?
    ...
    // ---------------------------------------------------------
    // CONEXIÓN CON TANK VISUALS (¡LA CLAVE!)
    // ---------------------------------------------------------
    void ActualizarAnimacion()
    {
        if (tankVisuals != null)
        {
            // Enviamos la dirección y si nos estamos moviendo
            // TankVisuals se encarga de calcular los ángulos (0, 90, 180, 270)
            tankVisuals.ActualizarVisuales(direccionMovimiento, moviendose);
        }
    }

    // ---------------------------------------------------------
    // LÓGICA DE PATHFINDING Y PUENTES (Replicada de tu código)
    // ---------------------------------------------------------
    ...
    void EncontrarRutaConPuentes(Vector3 destinoFinal)
    {
        ...
        // Simulación simplificada de búsqueda de puentes (Greedy)

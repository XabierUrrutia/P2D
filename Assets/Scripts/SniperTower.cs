...
/// <summary>
/// Torre de vigilância com comportamento AI simples:
/// - Detecta inimigos no alcance e escolhe o mais próximo.
/// - Checa linha de visão opcionalmente.
...
public class SniperTower : MonoBehaviour
{
    ...
    public float turretRotateSpeed = 10f;       // suavidade da rotação
    ...
    void Start()
    {
        // criar firePoint se não existir
        if (firePoint == null)
        {
    ...
    void FindTarget()
    {
        ...
        {
            // Fallback: procurar por tag no cenário (útil se LayerMask não foi configurado)
        ...
        // Evita mudar de alvo constantemente se já temos um alvo válido e a diferença é pequena
        if (best != null && currentTarget != null && best != currentTarget)
        {
            if (aimTimer > 0f)
            {
                // manter o target atual até o timer expirar
                return;
            }
            else
            {
    ...
    void ShootAt(Transform target)
    {
        ...
        // girar projétil para direção
        float angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg;
        bgo.transform.rotation = Quaternion.AngleAxis(angle, Vector3.forward);

        // opcional: destruir projétil após tempo (evita acumular)
        Destroy(bgo, 5f);

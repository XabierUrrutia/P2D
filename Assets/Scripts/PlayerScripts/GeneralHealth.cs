...
public class GeneralHealth : MonoBehaviour, IHealth
{
    ...
    void Update()
    {
        // Actualizar posición de las barras si están visibles
        if (!isDead)
        {
            if (healthBarVisible)
            {
    ...
    // Implementación de IHealth.TakeDamage - VERSIÓN CORREGIDA
    public void TakeDamage(int damage)
    {
        if (isDead || GameManager.Instance.IsGameOver()) return;

        Debug.Log($"[GeneralHealth] {name}: Recibiendo {damage} de daño. Escudo: {currentShield}/{maxShield}, Vida: {currentHealth}/{maxHealth}");

        int remainingDamage = damage;

        // Primero dañar el escudo si existe y no está roto
        if (hasShield && currentShield > 0 && !isShieldBroken)
        {
            Debug.Log($"[GeneralHealth] {name}: Escudo absorbiendo daño. Escudo antes: {currentShield}");
            ...
            Debug.Log($"[GeneralHealth] {name}: Escudo dañado: {shieldDamage}. Escudo después: {currentShield}, Daño restante: {remainingDamage}");

            // Si el escudo llegó a 0, marcarlo como roto
            if (currentShield <= 0)
            {
                isShieldBroken = true;
                currentShield = 0;
            ...
            shieldRegenTimer = shieldRegenDelay; // Reiniciar temporizador de regeneración
        }
        else if (hasShield && (currentShield <= 0 || isShieldBroken))
        {
            Debug.Log($"[GeneralHealth] {name}: Escudo ya está roto o agotado, daño va directo a vida");
        }
        ...
        if (remainingDamage > 0)
        {
            ...
            // Si recibió daño directo a la vida, reiniciar regeneración de escudo
            if (hasShield)
            {
    ...
    // Implementación de IHealth.RepairShield
    ...
    // Implementación de IHealth.GetCurrentShield
    public int GetCurrentShield()
    {
        return hasShield ? currentShield : 0;
    }

    // Implementación de IHealth.GetMaxShield
    ...
    // Implementación de IHealth.IsFullShield
    ...
    // Método para configurar como general
